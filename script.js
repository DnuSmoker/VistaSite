...
function updateCountdownSite() {
  ...
  const targetDateSite = new Date('January 25, 2025 20:00:00 GMT+0200').getTime(); // Bucharest timezone
...
function updateCountdownPublic() {
  ...
  function updateTime() {
    ...
    if (timeLeft > 0) {
      ...
      const minutes = Math.floor((timeLeft % (1000 * 60 * 60)) / (1000 * 60));
      const seconds = Math.floor((timeLeft % (1000 * 60)) / 1000);
      countdownPublicElement.textContent = `${String(days).padStart(2, '0')}:${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`;
    } else {
      countdownPublicElement.textContent = "00:00:00:00"; // Countdown finished
    }
  }
...
const translations = {
  ro: {
    ...
    site_update: "UPDATE LA SITE! | V0.4 ⚙️",
    public_launch: "LANSAREA SITE-ULUI PUBLIC! 🚀",
    map: "/MAP",
    ...
    countdown_text: "Timp rămas până la deschiderea serverului de Minecraft:",
    countdown_site_text: "Timp rămas până la update-ul site-ului:",
    countdown_public_text: "Timp rămas până la lansarea site-ului public:",
  },
  en: {
    title: "WELCOME",
    ...
    site_update: "SITE UPDATE! | V0.4 ⚙️",
    public_launch: "PUBLIC SITE LAUNCH! 🚀",
    map: "/MAP",
    ...
    countdown_text: "Time remaining until the Minecraft server opens:",
    countdown_site_text: "Time remaining until site update:",
    countdown_public_text: "Time remaining until public site launch:",
  }
};
...
function setLanguage(language) {
  ...
  document.getElementById('public_launch').textContent = lang.public_launch;
  ...
  document.getElementById('countdown_text').textContent = lang.countdown_text;
  document.getElementById('countdown_site_text').textContent = lang.countdown_site_text;
  document.getElementById('countdown_public_text').textContent = lang.countdown_public_text;
}
